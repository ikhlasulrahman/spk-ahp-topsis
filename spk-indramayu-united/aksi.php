...
elseif ($mod == 'alternatif_tambah') {
    ...
    elseif ($db->get_results("SELECT * FROM tb_alternatif WHERE kode_alternatif='$kode' and id_lini = '$ID_LINI'"))
        ...
        $db->query("INSERT INTO tb_rel_alternatif(id_lini, kode_alternatif, kode_kriteria, nilai) SELECT '$ID_LINI', '$kode', kode_kriteria, 0 FROM tb_kriteria where id_lini = '$ID_LINI'");
...
elseif ($mod == 'kriteria_tambah') {
    ...
    elseif ($db->get_results("SELECT * FROM tb_kriteria WHERE kode_kriteria='$kode' and id_lini = '$ID_LINI'"))
        ...
        $db->query("INSERT INTO tb_rel_alternatif(id_lini, kode_alternatif, kode_kriteria, nilai) SELECT '$ID_LINI', kode_alternatif, '$kode', 0  FROM tb_alternatif where id_lini = '$ID_LINI'");
...
/** RELASI ALTERNATIF */
    ...
    foreach ($_POST as $key => $value) {
        ...
        $db->query("UPDATE tb_rel_alternatif SET nilai='$value' WHERE ID='$ID' and id_lini=$ID_LINI");
    }
    set_message('Nilai bobot alternatif berhasil diubah!', 'success');
    redirect_js("index.php?m=rel_alternatif&id_lini=$ID_LINI");
}
