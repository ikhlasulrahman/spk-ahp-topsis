...
// ===== AWAL KODE KONTROL AKSI =====
if (isset($_SESSION['level'])) {
    $level = $_SESSION['level'];
    $mod = get('mod');
    $act = get('act');

    // Manajer (level 2) tidak boleh melakukan aksi apa pun kecuali logout
    if ($level == '2' && $act != 'logout' && $act != 'login') {
    ...
    // Admin (level 3) hanya boleh aksi akun dan password
    $admin_allowed_mods = ['akun_tambah', 'akun_ubah', 'password'];
    $admin_allowed_acts = ['akun_hapus'];
    if ($level == '3' && !in_array($mod, $admin_allowed_mods) && !in_array($act, $admin_allowed_acts) && $act != 'logout' && $act != 'login') {
        set_message('Admin tidak memiliki hak akses untuk aksi ini.', 'danger');
        redirect_js('index.php?m=akun');
        exit;
    }

    // Pelatih (level 1) tidak boleh aksi akun
    $pelatih_disallowed_mods = ['akun_tambah', 'akun_ubah'];
    $pelatih_disallowed_acts = ['akun_hapus'];
    if ($level == '1' && (in_array($mod, $pelatih_disallowed_mods) || in_array($act, $pelatih_disallowed_acts))) {
        set_message('Pelatih tidak memiliki hak akses untuk mengelola akun.', 'danger');
        redirect_js('index.php?m=home');
        exit;
    }
}
// ===== AKHIR KODE KONTROL AKSI =====
