...
$lini_id = $ID_LINI;
$nama_lini = $db->get_var("SELECT nama FROM tb_lini WHERE id_lini='$lini_id'");
$kebutuhan = (int) $db->get_var("SELECT kebutuhan FROM tb_lini WHERE id_lini='$lini_id'");

$alternatifs = $db->get_results("SELECT * FROM tb_alternatif WHERE id_lini='$lini_id' ORDER BY rank");
$kriterias = $db->get_results("SELECT kode_kriteria, nama_kriteria FROM tb_kriteria WHERE id_lini = '$lini_id' ORDER BY kode_kriteria");
$nilai_alternatif = TOPSIS_get_hasil_analisa();

// Mengambil data pemain terpilih sesuai kebutuhan
$pemain_terpilih = [];
if ($kebutuhan > 0) {
    $pemain_terpilih = $db->get_results("SELECT * FROM tb_alternatif WHERE id_lini='$lini_id' ORDER BY rank ASC LIMIT $kebutuhan");
}
?>
    ...
    <tbody>
        <?php foreach ($alternatifs as $alternatif) : ?>
            <tr>
                ...
                <td><?= htmlspecialchars($alternatif->nama_alternatif) ?></td>
                <td><?= htmlspecialchars($alternatif->jabatan) ?></td>
        ...
        <?php if (!empty($pemain_terpilih)): ?>
            <?php foreach ($pemain_terpilih as $pemain) : ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($pemain->jabatan) ?></td>
                    <td><?= round($pemain->total, 4) ?></td>
                </tr>
            <?php endforeach; ?>
        <?php else: ?>
            <tr>
                <td colspan="5" style="text-align:center;">Data pemain terpilih tidak tersedia.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
